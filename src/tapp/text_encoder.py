...
class SentimentTextEncoder(TextEncoder):
    def __init__(self):
        print("Loading sentiment model...")
        self.tokenizer, self.model = self._load_sentiment_model()
        print("Sentiment model loaded.")
        super().__init__(encoding_length=3)
    ...
    def fit(self, docs):
        # No fitting needed for sentiment analysis
        return self
    ...
    def _get_sentiment_vector(self, text):
        ...
        with torch.no_grad():
            ...
            return self._softmax(scores)  # returns [neg, neu, pos] vector

    def transform(self, docs):
        return np.array([self._get_sentiment_vector(doc) for doc in docs])
...
class BERTbaseTextEncoder(TextEncoder):
    ...
    def transform(self, docs):
        ...
        # Stack embeddings into a 2D array: (num_docs, embedding_dim)
        embeddings = np.vstack(list_embeddings)

        # If only one document, still return shape (1, embedding_dim)
        return embeddings
    ...
    def bert_text_preparation(self, text):
        ...
        tokens_tensor = torch.tensor([indexed_tokens]).to(self.device)   # move to GPU
        segments_tensors = torch.tensor([segments_ids]).to(self.device)  # move to GPU
    ...
    def get_bert_embeddings(self, tokens_tensor, segments_tensors, embedding_level="word"):
        with torch.no_grad():
            ...
            hidden_states = outputs[2][1:]  # skip input embeddings
