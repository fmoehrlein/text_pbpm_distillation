...
try:
    # Try old import (works in older TF versions)
    from keras.losses import mean_absolute_error, mean_squared_error
except ImportError:
    # New TF versions: import from the losses submodule
...
class TappModel(PredictionModel):
    ...
    def __init__(self, log_encoder=None, num_shared_layer=1, num_specialized_layer=1, neurons_per_layer=100, dropout=0.2,
                 learning_rate=0.002, use_lr_reduction=True):
        ...
        self.feature_dim = 0
        self.model = None
        self.use_lr_reduction = use_lr_reduction
        super().__init__()
    ...
    def fit_with_ready_data(self, x, y_next_act, y_next_time, epochs=100, validation_split=0.2, patience_rl=5,
            patience_es=10, batch_size=None):
        # Build model
        self.timesteps = x.shape[1]
        self.feature_dim = x.shape[2]
        print("building model with timesteps =", self.timesteps, "and feature_dim =", self.feature_dim)
        self._build_model()
    ...
    def fit(self, log, data_attributes=None, text_attribute=None, epochs=100, validation_split=0.2, patience_rl=5,
            patience_es=20, batch_size=None):
